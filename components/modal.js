...
export default function ModalContent({ onClose, onTitleEdit, title, addComment }) {
    ...
    return (
        <div className="fixed inset-0 z-10 modal-container backdrop-blur-md">
            <div className="fixed p-4 -translate-x-1/2 -translate-y-1/2 bg-white rounded-md w-80 text-end modal top-1/2 left-1/2">
                ...
                <form className="flex flex-col gap-4" ref={formComments} action="" onSubmit={handlePushComment}>
                    ...
                    <input className="px-1 text-black bg-gray-200" autoComplete="off" type="text" name="newComment" placeholder="Añadir comentario"/>
                    <button className="p-2 text-white rounded bg-sky-600">Comentar</button>
                </form>
            </div>
        </div>
    );
